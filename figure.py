    ...
    if (county, state) == ('New York City', 'New York'):
        for fips_code, county in zip(nyc_fips, nyc_counties):
            if not fips_code in nyc_fips_codes_seen:
                corona_dict[fips_code] = ['New York', county, fips2pops[fips_code], [(parse_time(date), (cases, deaths))]]
        ...
        if not fips_code in fips_codes_seen:
            corona_dict[fips_code] = [state, county, fips2pops[fips_code], [(parse_time(date), (cases, deaths))]]
...
color_weights = [nozeroerrorlog(float(x)) for x in corona_df['Cases per 100,000 Population']]

#mapper = px.choropleth
mapper = px.choropleth_mapbox

mapper_kwargs = {px.choropleth: {'scope': 'usa',},
                 px.choropleth_mapbox: {'mapbox_style': 'carto-positron', 'zoom': 3,
